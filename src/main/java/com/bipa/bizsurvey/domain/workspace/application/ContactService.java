...
public class ContactService {
    ...
    public ContactDto.Response create(ContactDto.CreateRequest request) {
        Workspace workspace =  workspaceRepository.findWorkspaceByIdAndDelFlagFalse(request.getWorkspaceId()).orElseThrow(() -> new RuntimeException("존재하지 않는 워크스페이스 입니다."));
    ...
    public ContactDto.Response readOne(Long id) {
        Contact contact = contactRepository.findByIdAndDelFlagFalse(id).orElseThrow(() -> new RuntimeException("존재하지 않는 연락처입니다."));
        return ContactDto.Response.builder()
                .id(contact.getId())
                .name(contact.getName())
                .email(contact.getEmail())
    ...
    public List<ContactDto.Response> searchContacts(ContactDto.SearchRequest request) {
        List<Contact> list = contactRepository.searchContact(request.getWorkspaceId(),
                                                             request.getKeyword());

        log.info("request: " + request);
        log.info("list: " + list.size());

        List<ContactDto.Response> result =  list.stream().map(e -> ContactDto.Response.builder()
                .id(e.getId())
                .name(e.getName())
                .email(e.getEmail())
                .remark(e.getRemark())
    ...
    public void delete(Long id) {
        Contact contact = contactRepository.findById(id).orElseThrow(() -> new RuntimeException("존재하지 않는 연락처입니다."));
        contact.delete();
    }
}
